...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "changedetection-pp-cli",
		Short: `Changedetection CLI — Every changedetection.io watch, tag, and notification from one CLI — plus offline cross-watch queries the web UI can't …`,
		Long: `Changedetection CLI — Every changedetection.io watch, tag, and notification from one CLI — plus offline cross-watch queries the web UI can't …
...
Highlights (not in the official API docs):
  ...
  • overdue   List watches past their scheduled recheck time.
...
Health check: run 'changedetection-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("changedetection-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'changedetection-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelErroredCmd(flags))
	...
	rootCmd.AddCommand(newNovelWatchSearchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBulkImportPromotedCmd(flags))
	rootCmd.AddCommand(newFindPromotedCmd(flags))
	rootCmd.AddCommand(newFullSpecPromotedCmd(flags))
	rootCmd.AddCommand(newSysteminfoPromotedCmd(flags))
