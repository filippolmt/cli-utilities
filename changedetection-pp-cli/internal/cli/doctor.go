...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  changedetection-pp-cli doctor
  changedetection-pp-cli doctor --json
  changedetection-pp-cli doctor --fail-on warn
  changedetection-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set one of: export CHANGEDETECTION_API_KEY=\"your-token-here\" or export CHANGEDETECTION_IO_API_KEY=\"your-token-here\""
					report["auth_docs_url"] = "https://github.com/dgtlmoon/changedetection.io"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CHANGEDETECTION_API_KEY")
				if os.Getenv("CHANGEDETECTION_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "CHANGEDETECTION_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CHANGEDETECTION_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CHANGEDETECTION_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "CHANGEDETECTION_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CHANGEDETECTION_IO_API_KEY")
				if os.Getenv("CHANGEDETECTION_IO_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "CHANGEDETECTION_IO_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CHANGEDETECTION_IO_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CHANGEDETECTION_IO_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "CHANGEDETECTION_IO_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "changedetection-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("changedetection-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'changedetection-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'changedetection-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'changedetection-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'changedetection-pp-cli sync' to refresh."
	}
	return report
}
