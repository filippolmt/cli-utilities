...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  changedetection-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  changedetection-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  changedetection-pp-cli workflow status

  # Show status as JSON
  changedetection-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("changedetection-pp-cli")
			}
