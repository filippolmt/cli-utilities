...
In local mode: searches locally synced data only.`,
  ...
  changedetection-pp-cli search "error timeout"

  # Force local search only
  changedetection-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  changedetection-pp-cli search "status" --type bulk_import --data-source local
  # JSON output for piping
  changedetection-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'changedetection-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "tag":
				results, err = db.SearchTag(query, limit)
			case "watch":
				results, err = db.SearchWatch(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchTag(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tag failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchWatch(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search watch failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
