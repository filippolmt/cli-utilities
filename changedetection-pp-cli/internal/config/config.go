...
type Config struct {
	...
	ChangedetectionApiKey   string          `toml:"api_key"`
	ChangedetectionIoApiKey string          `toml:"io_api_key"`
	// InsecureTLS skips TLS certificate verification. Opt-in via
	// CHANGEDETECTION_INSECURE or insecure_tls in config, for self-signed
	// instances (equivalent to curl -k). Off by default.
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("CHANGEDETECTION_API_KEY"); v != "" {
		cfg.ChangedetectionApiKey = v
		cfg.markEnvOverride("ChangedetectionApiKey")
		cfg.AuthSource = "env:CHANGEDETECTION_API_KEY"
		cfg.CredentialSource = "env:CHANGEDETECTION_API_KEY"
	}
	if v := os.Getenv("CHANGEDETECTION_IO_API_KEY"); v != "" {
		cfg.ChangedetectionIoApiKey = v
		cfg.markEnvOverride("ChangedetectionIoApiKey")
		cfg.AuthSource = "env:CHANGEDETECTION_IO_API_KEY"
		cfg.CredentialSource = "env:CHANGEDETECTION_IO_API_KEY"
	}
	...
	if v := os.Getenv("CHANGEDETECTION_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	// Skip TLS verification for self-signed instances (opt-in).
	switch strings.ToLower(strings.TrimSpace(os.Getenv("CHANGEDETECTION_INSECURE"))) {
	case "1", "true", "yes", "on":
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CHANGEDETECTION_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "changedetection-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ChangedetectionApiKey != "" {
		return c.ChangedetectionApiKey
	}
	if c.ChangedetectionIoApiKey != "" {
		return c.ChangedetectionIoApiKey
	}
	return ""
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.ChangedetectionApiKey = ""
	c.ChangedetectionIoApiKey = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ChangedetectionApiKey:   c.ChangedetectionApiKey,
		ChangedetectionIoApiKey: c.ChangedetectionIoApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ChangedetectionApiKey = creds.ChangedetectionApiKey
	c.ChangedetectionIoApiKey = creds.ChangedetectionIoApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ChangedetectionApiKey = token
	delete(c.envOverrides, "ChangedetectionApiKey")
	c.updateFileConfigField("ChangedetectionApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ChangedetectionApiKey")
	c.ChangedetectionIoApiKey = ""
	delete(c.envOverrides, "ChangedetectionIoApiKey")
	c.updateFileConfigField("ChangedetectionIoApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ChangedetectionApiKey"] {
			out.ChangedetectionApiKey = c.fileConfig.ChangedetectionApiKey
		}
		if c.envOverrides["ChangedetectionIoApiKey"] {
			out.ChangedetectionIoApiKey = c.fileConfig.ChangedetectionIoApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ChangedetectionApiKey":
		c.fileConfig.ChangedetectionApiKey = c.ChangedetectionApiKey
	case "ChangedetectionIoApiKey":
		c.fileConfig.ChangedetectionIoApiKey = c.ChangedetectionIoApiKey
	}
}
