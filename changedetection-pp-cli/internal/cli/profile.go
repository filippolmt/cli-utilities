...
func legacyProfileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".changedetection-pp-cli")
	return filepath.Join(dir, "profiles.json"), nil
}
...
captured: they control profile/config resolution and would never apply).`,
		Example: `  changedetection-pp-cli profile save my-defaults --json --compact
  changedetection-pp-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  changedetection-pp-cli profile use my-defaults
  changedetection-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  changedetection-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  changedetection-pp-cli profile show my-defaults
  changedetection-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  changedetection-pp-cli profile delete my-defaults --yes
  changedetection-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
