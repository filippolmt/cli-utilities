...
Resource scoping:
  ...
  changedetection-pp-cli sync

  # Sync specific resources only
  changedetection-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  changedetection-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  changedetection-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  changedetection-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  changedetection-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "difference":
		return db.UpsertDifference(data)
	case "favicon":
...
func responsePathForResource(resource, path string) []string {
	...
	case "notifications\x00/notifications":
		return []string{"notification_urls"}
	}
	return nil
}
