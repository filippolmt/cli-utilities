...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		...
		"CHANGEDETECTION_CONFIG_DIR",
		"CHANGEDETECTION_DATA_DIR",
		"CHANGEDETECTION_STATE_DIR",
		"CHANGEDETECTION_CACHE_DIR",
		"CHANGEDETECTION_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
		"CHANGEDETECTION_API_KEY",
		"CHANGEDETECTION_IO_API_KEY",
	} {
		t.Setenv(name, "")
	}
	...
	return home, filepath.Join(home, ".config", "changedetection-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "changedetection-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("CHANGEDETECTION_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "changedetection-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "changedetection-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "changedetection-pp-cli", "credentials.toml")
...
func configCredentialValue(cfg *config.Config) string {
	return cfg.ChangedetectionApiKey
}
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.ChangedetectionApiKey
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.ChangedetectionApiKey = token
}
