...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'changedetection-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set one of: export CHANGEDETECTION_API_KEY=\"your-token-here\" or export CHANGEDETECTION_IO_API_KEY=\"your-token-here\""+
			"\n      See API docs: https://github.com/dgtlmoon/changedetection.io"+
			"\n      Run 'changedetection-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set one of: export CHANGEDETECTION_API_KEY=\"your-token-here\" or export CHANGEDETECTION_IO_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
