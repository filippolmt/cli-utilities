...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CHANGEDETECTION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CHANGEDETECTION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "changedetection-pp-cli/feedback")
...
When ` + "`CHANGEDETECTION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CHANGEDETECTION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  changedetection-pp-cli feedback list
  changedetection-pp-cli feedback list --limit 5
  changedetection-pp-cli feedback list --json`,
