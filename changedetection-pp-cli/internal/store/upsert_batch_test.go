...
func TestSearchTagQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("tag", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchTag(query, 10)
		if err != nil {
			t.Fatalf("SearchTag(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchTag(%q) returned no results", query)
		}
	}
}
...
func TestSearchWatchQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("watch", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchWatch(query, 10)
		if err != nil {
			t.Fatalf("SearchWatch(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchWatch(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesDifferenceTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed difference table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDifferenceTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "watch_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "watch_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "watch_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("difference", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "difference").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "difference")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count difference: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("difference count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandDifferenceGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandDifferenceGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "watch_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("difference", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("difference count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "watch_id")
	}
}

// TestUpsertBatch_KeysDifferenceByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysDifferenceByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "watch_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "watch_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "watch_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "watch_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("difference", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("difference count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "watch_id" = ?`, "difference")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by watch_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("watch_id=parent_A count = %d, want 2", parentMatches)
	}
}
...
// TestUpsertBatch_TypedFailureDoesNotStrandFaviconGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandFaviconGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("favicon", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("favicon count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "watch_id")
	}
}

// TestUpsertBatch_KeysFaviconByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysFaviconByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("favicon count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "watch_id" = ?`, "favicon")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by watch_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_TypedFailureDoesNotStrandHistoryGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("history count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "watch_id")
	}
}
...
func TestUpsertBatch_KeysHistoryByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "watch_id" = ?`, "history")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by watch_id: %v", err)
	}
	if parentMatches != 2 {
