...
func newHTTPClient(timeout time.Duration, jar http.CookieJar, insecure bool) *http.Client {
	...
	if insecure {
		...
		// ponytail: opt-in (CHANGEDETECTION_INSECURE / insecure_tls) for
		// self-signed instances; equivalent to curl -k.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient := newHTTPClient(timeout, nil, cfg != nil && cfg.InsecureTLS)
	c := &Client{
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "changedetection-pp-cli/0.1.7")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export CHANGEDETECTION_API_KEY=<your-token> or changedetection-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ChangedetectionApiKey)
		addCredential(c.Config.ChangedetectionIoApiKey)
	}
