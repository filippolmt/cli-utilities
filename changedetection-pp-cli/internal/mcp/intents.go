...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("read_the_latest_change",
			mcplib.WithDescription("Print the unified text diff of a watch's two most recent snapshots."),
			mcplib.WithString("uuid", mcplib.Required(), mcplib.Description("Override the recipe's positional uuid value.")),
		),
		handleReadTheLatestChange,
	)
}

// handleReadTheLatestChange runs the read_the_latest_change recipe intent: Print the unified text diff of a watch's two most recent snapshots.
func handleReadTheLatestChange(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingUuid bool
	args, missingUuid = appendRecipePositional(args, input["uuid"], true)
	if missingUuid {
		return mcplib.NewToolResultError("uuid is required"), nil
	}
