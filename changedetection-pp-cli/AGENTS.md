# changedetection.io Printed CLI Agent Guide

This directory is a generated `changedetection-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
changedetection-pp-cli doctor --json
changedetection-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
changedetection-pp-cli which "<capability>" --json
changedetection-pp-cli <command> --help
```
...
```bash
changedetection-pp-cli <command> --agent
```
...
```bash
changedetection-pp-cli <command> --help
changedetection-pp-cli <command> --dry-run --agent
```
